...
def create_location3():
    ...
    for i, r in loc1.iterrows():
        ...
        loc2["LA"] = r.LATITUDE
        loc2["LO"] = r.LONGITUDE
        loc2["d"] = loc2.apply(lambda x: haversine(x.LONGITUDE, x.LATITUDE, x.LO, x.LA), axis=1)
        near = loc2.sort_values("d").pref.values
        near = list(near)
        loc1.at[i, "near_prefs"] = near
    ...
    df["d"] = df.apply(lambda x: haversine(x.LONGITUDE, x.LATITUDE, x.LONGITUDE_u, x.LATITUDE_u), axis=1)
...
def create_pref_to_id():
    ...
    loc4 = loc4.sort_values(["z_large_area", "z"]).reset_index(drop=True)
